...
export default function RegisterPage() {
    ...
    const onSubmit = async (data: RegisterFormValues) => {
        try {
            ...
            // Update the profile table directly (optional, Supabase triggers often handle this)
            if (authData.user) {
                await supabase.from('profiles').upsert({
                    id: authData.user.id,
                    kyc_full_name: data.fullName,
                    role: 'referrer',
                    org_id: '0ff27f8e-48cd-4c70-9bfc-219ff98ea5ae' // Default org id
                });
            }
    ...
    return (
        <div className="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-background relative selection:bg-primary/20">
            {/* Clean Background */}

            <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
                <div className="flex justify-center mb-6">
                    <img src="/logo.png" alt="Gainux Logo" className="h-14 rounded-xl shadow-sm" />
                </div>
                <h2 className="mt-2 text-center text-3xl font-extrabold tracking-tight text-foreground">
                    Join Gainux
                </h2>
                <p className="mt-2 text-center text-sm text-muted-foreground">
                    Create an account to become a referrer
                </p>
            </div>

            <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-[440px] relative z-10 transition-all duration-300">
                <div className="bg-card border border-border/60 shadow-sm py-10 px-4 sm:rounded-2xl sm:px-10">
                    <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
                        <div>
                            ...
                            <div className="mt-2">
                                ...
                                />
                                {errors.fullName && (
                                    <p className="mt-1.5 text-xs font-medium text-destructive">{errors.fullName.message}</p>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="mt-8">
                        ...
                        <div className="mt-6">
                            <Button
                                type="button"
                                variant="outline"
                                className="w-full h-11 bg-background hover:bg-secondary border-border/50 text-foreground transition-all duration-200"
